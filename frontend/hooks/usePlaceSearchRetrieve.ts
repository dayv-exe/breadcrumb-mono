...
export const usePlaceSearchRetrieve = (sessionToken: string,
  onPlaceFound: (coords: [number, number]) => void): returnType => {
  const [placeId, setPlaceId] = useState("")
  const { data: placeInfo, isError: retrieveFailed, isPending: retrievePending } = useRetrievePlace(sessionToken, placeId)

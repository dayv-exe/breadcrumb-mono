...
func HandleMediaAccess(ctx context.Context, req events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	case "presign":
		isProfilePicture := strings.ToLower(strings.TrimSpace(req.QueryStringParameters["profilePicture"])) == "true"

		if isProfilePicture {
			return HandlePresignProfilePictureUrl(ctx, req)
		}
		return HandlePresignCrumbMediaUrl(ctx, req)
